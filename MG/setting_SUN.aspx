...
Sub Page_Load()
	...
	If Request("FCMD") <> "" Then
		...
		Case "Save"
			RS = getRecord("SELECT TYPE FROM SYS_CODE_SUN WHERE TYPE LIKE 'TES_[VNQ]%'")
			While RS.Read
				S &= "UPDATE SYS_CODE_SUN SET CODE=" & chkRN(RS(0)) & " WHERE TYPE='" & RS(0) & "'" & vbLf
			End While
			RS.Close

			RS = getRecord("SELECT TYPE FROM SYS_ENUM_SUN  WHERE TYPE NOT IN('DISC','TRD_LVL','OBSV','MTG_ID') AND TYPE NOT LIKE 'FXP_%'")
			While RS.Read
				If chkR9(RS(0)) <> "NULL" Then
					S &= "UPDATE SYS_ENUM_SUN SET NUM=" & chkR9(RS(0)) & IIf("CMB, WIS BLN MKT".Contains(Left(RS(0), 3)), " * 100", "") & " WHERE TYPE='" & RS(0) & "'" & vbLf
				End If
			End While
			RS.Close

			RS = getRecord("SELECT LVL_ID FROM SYS_LEVEL_SUN  ORDER BY LVL_ID")
			While RS.Read
				L = ""
				For I = 1 To 11
					L &= ", LVL_" & I & "=" & ChkR9("LVL_" & RS(0) & "_" & I)
				Next
				S &= "UPDATE SYS_LEVEL_SUN SET" & Mid(L, 2) & " WHERE LVL_ID=" & RS(0) & vbLf
			End While
			RS.Close

			RS = getRecord("SELECT COUNTRY, TYPE FROM SYS_BIAS_SUN  ORDER BY COUNTRY, TYPE")
			While RS.Read
				L = ""
				For I = 1 To 10
					L &= ", LSB_" & I & "=" & ChkR9("LSB_" & RS(0) & RS(1) & "_" & I)
				Next
				S &= "UPDATE SYS_BIAS_SUN SET" & Mid(L, 2) & " WHERE COUNTRY='" & RS(0) & "' AND TYPE='" & RS(1) & "'" & vbLf
			End While
			RS.Close

			RS = getRecord("SELECT COUNTRY, TYPE FROM SYS_FVT_SUN  ORDER BY COUNTRY, TYPE")
			While RS.Read
				L = RS(0) & RS(1)
				S &= "UPDATE SYS_FVT_SUN SET MAX_MDL=" & ChkR9("MXM_" & L) & _
		...
		Case "Update"
			...
			RS = getRecord("SELECT ROW_ID FROM SYS_WISE_SUN  ORDER BY CUST_ID, REGION")
			While RS.Read
				L = ""
				For I = 1 To 11
				...
				Next
				S &= "UPDATE SYS_WISE_SUN SET " & Mid(L, 2) & " WHERE ROW_ID=" & RS(0) & vbLf
			End While
			RS.Close
		
		Case "New"											'Add new Customer to WISE table
			If ChkR9("NEWID") <> "NULL" THEN
				S &= "INSERT SYS_WISE_SUN (CUST_ID,REGION) VALUES (" & ChkR9("NEWID") & ",'M')" & vbLf
				S &= "INSERT SYS_WISE_SUN (CUST_ID,REGION) VALUES (" & ChkR9("NEWID") & ",NULL)" & vbLf
			ELSE
		...
		'response.write(s)
		execSQL(S)
		Application.Remove("SYS_CODE_SUN")
		Application.Remove("SYS_ENUM_SUN")
		Application.Remove("SYS_BIAS_SUN")
	End If
...
<html>
	...
	<script>top.setTitle("Parameter Settings - SUN")</script>
				...
				<table width=1100 cellpadding=0>
					...
					<div class=LST>
						<table cellspacing=0>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE LIKE 'DHV%' ORDER BY TYPE")
							While RS.Read  %>
								<tr><% 
					...
					<div class="LST GAP">
						<table>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE LIKE 'PCT%' ORDER BY TYPE")
							While RS.Read  %>
								<tr><% 
					...
					<div class="LST GAP" id=WIP>
					    <table>
					        ...
					        <%
							RS = getRecord("SELECT * FROM SYS_ENUM_SUN (nolock) WHERE TYPE LIKE 'MKTPCT_%' ORDER BY [TYPE]")
							While RS.Read  %>
					        <tr>
					...
					<div class=LST>
						<table>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE LIKE 'RDC_ST_%' ORDER BY TYPE")
							While RS.Read		%>
								<tr>
					...
					<div class="LST GAP" id=WIP>
						<table>
							...
							<tr><th colspan=3 class=TT>Other %<%
								RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE LIKE 'CMB_%' OR TYPE LIKE 'BLN_%' ORDER BY LEFT(TYPE,3) DESC, TYPE")
								While RS.Read  %>
								<tr><% 
									If Right(RS(0), 1) = 1 Then %>
							...
							<tr>
								...
								RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE IN('MRR_MTP','MRR_PC1','MRR_PC2') ORDER BY TYPE")
								While RS.Read  %>
									<tr><% 
							...
							<tr>
								<td colspan=3 class=SPT><%
								RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE IN('1ST_UP_G','OVL_VWM','XB1_MKP','DVP_SKW','DVP_SKW_RP','DVP_SKW_RC','DVP_SKW_G','DVP_SKW_H','CFLA_SKW','CFLB_SKW','CFLC_SKW','CFLD_SKW' ) ORDER BY VALUE")
								While RS.Read  %>
									<tr>
										<td colspan=2><%= RS(2) %>
										<td><input name=<%= RS(0) %> type=number value=<%= RS(1) %>><% 
							...
							<tr>
								<td colspan=3 class=SPT><%
								RS = getRecord("SELECT * FROM SYS_ENUM_SUN (nolock) WHERE TYPE IN( 'WISECL_A',  'WISECL_B', 'WISECL_C',  'WISECL_D')")							
								While RS.Read  %>
									<tr>
										<td colspan=2><%= RS(2) %>
										<td><input name=<%= RS(0) %> type=number  step='0.01' value=<%= FormatNumber(RS(1) / 100)  %>><% 
					...
					<div class="LST GAP">
						<table>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM_SUN  WHERE TYPE LIKE 'KLY%' ORDER BY TYPE")
								While RS.Read%>
									<tr>
					...
					<div class=LST>
						<table>
							...
							<%
							RS = getRecord("SELECT * FROM SYS_LEVEL_SUN  ORDER BY LVL_ID")
							While RS.Read  %>
								<tr>
					...
					<div class="LST GAP">
						<table>
							...
							<%
							RS = getRecord("SELECT * FROM SYS_BIAS_SUN  ORDER BY CASE COUNTRY WHEN 'XX' THEN 1 ELSE 0 END, COUNTRY, TYPE")
							While RS.Read			%>
								<tr>
								<% If RS(0) <> "XX" Then %>
					...
					<div class="LST GAP" style="width:350px">
						<table>
							...
							<%
							RS = getRecord("SELECT * FROM SYS_FVT_SUN  ORDER BY CASE COUNTRY WHEN 'XX' THEN 1 ELSE 0 END, COUNTRY, TYPE")
							While RS.Read		  %>
								<tr>
								<% If RS(0) <> "XX" Then %>
									<td><img align=absmiddle class=FLG src="/img/<%= RS(0) %>.jpg"> <img align=absmiddle src="/img/<%= RS(1) %>.png">
					...
					<div class="LST GAP" style="width:270px">
						...
						<%	RS = getRecord("SELECT * FROM SYS_CODE_SUN WHERE TYPE LIKE 'TES_[VNQ]%' ORDER BY CHARINDEX(LEFT(NAME,1),'VNQ')")
						While RS.Read		  %>
						  <tr>
							<td><%= RS(2) %>
							<td><select name=<%= RS(0) %> style="width:92%"><%= makeList("SELECT", "", "SYS_CODE_SUN", "TES_LST", "CODE", "NAME", RS(1)) %></select> <%
						End While
						RS.Close %>
						</table>
					</div>
				...
				<div id=CNT style="top:25px; left:1125px">
							...
							<div class=LST>
								<table>
									...
									<%
									RS = getRecord("SELECT * FROM SYS_WISE_SUN  ORDER BY CUST_ID, REGION")
									While RS.Read  %>
										<tr>
											<td><b><%= RS(1) %>
											<td><b><%= RS(2) %>
