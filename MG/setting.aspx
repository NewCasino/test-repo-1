...
Sub Page_Load()
	...
	If Request("FCMD") <> "" Then
		...
		Case "Save"
			RS = getRecord("SELECT TYPE FROM SYS_CODE(nolock) WHERE TYPE LIKE 'TES_[VNQ]%'")
			While RS.Read
				S &= "UPDATE SYS_CODE SET CODE=" & chkRN(RS(0)) & " WHERE TYPE='" & RS(0) & "'" & vbLf
			End While
			RS.Close

			RS = getRecord("SELECT TYPE FROM SYS_ENUM(nolock) WHERE TYPE NOT IN('DISC','TRD_LVL','OBSV','MTG_ID') AND TYPE NOT LIKE 'FXP_%'")
			While RS.Read
			...
			RS = getRecord("SELECT LVL_ID FROM SYS_LEVEL(nolock) ORDER BY LVL_ID")
			While RS.Read
				L = ""
				For I = 1 To 11
					L &= ", LVL_" & I & "=" & ChkR9("LVL_" & RS(0) & "_" & I)
				Next
				S &= "UPDATE SYS_LEVEL SET" & Mid(L, 2) & " WHERE LVL_ID=" & RS(0) & vbLf
			End While
			RS.Close

			RS = getRecord("SELECT COUNTRY, TYPE FROM SYS_BIAS(nolock) ORDER BY COUNTRY, TYPE")
			While RS.Read
				L = ""
				For I = 1 To 10
					L &= ", LSB_" & I & "=" & ChkR9("LSB_" & RS(0) & RS(1) & "_" & I)
				Next
				S &= "UPDATE SYS_BIAS SET" & Mid(L, 2) & " WHERE COUNTRY='" & RS(0) & "' AND TYPE='" & RS(1) & "'" & vbLf
			End While
			RS.Close

			RS = getRecord("SELECT COUNTRY, TYPE FROM SYS_FVT(nolock) ORDER BY COUNTRY, TYPE")
			While RS.Read
				L = RS(0) & RS(1)
				S &= "UPDATE SYS_FVT SET MAX_MDL=" & ChkR9("MXM_" & L) & _
		...
		Case "Update"
			...
			RS = getRecord("SELECT ROW_ID FROM SYS_WISE(nolock) ORDER BY CUST_ID, REGION")
			While RS.Read
				L = ""
				For I = 1 To 11
				...
				Next
				S &= "UPDATE SYS_WISE SET " & Mid(L, 2) & " WHERE ROW_ID=" & RS(0) & vbLf
			End While
			RS.Close
		
		Case "New"											'Add new Customer to WISE table
			If ChkR9("NEWID") <> "NULL" THEN
				S &= "INSERT SYS_WISE (CUST_ID,REGION) VALUES (" & ChkR9("NEWID") & ",'M')" & vbLf
				S &= "INSERT SYS_WISE (CUST_ID,REGION) VALUES (" & ChkR9("NEWID") & ",NULL)" & vbLf
			ELSE
		...
		'response.write(s)
		execSQL(S)
		Application.Remove("SYS_CODE")
		Application.Remove("SYS_ENUM")
		Application.Remove("SYS_BIAS")
	End If
...
<html>
	...
	<body>
				...
				<table width=1100 cellpadding=0>
					...
					<div class=LST>
						<table cellspacing=0>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM(nolock) WHERE TYPE LIKE 'DHV%' ORDER BY TYPE")
							While RS.Read  %>
								<tr><% 
					...
					<div class="LST GAP">
						<table>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM(nolock) WHERE TYPE LIKE 'PCT%' ORDER BY TYPE")
							While RS.Read  %>
								<tr><% 
					...
					<div class=LST>
						<table>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM(nolock) WHERE TYPE LIKE 'RDC_ST_%' ORDER BY TYPE")
							While RS.Read		%>
								<tr>
					...
					<div class="LST GAP" id=WIP>
						<table>
							...
							<tr><th colspan=3 class=TT>Other %<%
								RS = getRecord("SELECT * FROM SYS_ENUM(nolock) WHERE TYPE LIKE 'CMB_%' OR TYPE LIKE 'BLN_%' ORDER BY LEFT(TYPE,3) DESC, TYPE")
								While RS.Read  %>
								<tr><% 
									If Right(RS(0), 1) = 1 Then %>
							...
							<tr>
								...
								RS = getRecord("SELECT * FROM SYS_ENUM(nolock) WHERE TYPE IN('MRR_MTP','MRR_PC1','MRR_PC2') ORDER BY TYPE")
								While RS.Read  %>
									<tr><% 
							...
							<tr>
								<td colspan=3 class=SPT><%
								RS = getRecord("SELECT * FROM SYS_ENUM WHERE TYPE IN('1ST_UP_G','OVL_VWM','XB1_MKP','DVP_SKW','DVP_SKW_RP','DVP_SKW_RC','DVP_SKW_G','DVP_SKW_H' ,'CFLA_SKW','CFLB_SKW','CFLC_SKW','CFLD_SKW') ORDER BY VALUE")
								While RS.Read  %>
									<tr>
										<td colspan=2><%= RS(2) %>
										<td><input name=<%= RS(0) %> type=number value=<%= RS(1) %>><% 
					...
					<div class="LST GAP">
						<table>
							...
							RS = getRecord("SELECT * FROM SYS_ENUM(nolock) WHERE TYPE LIKE 'KLY%' ORDER BY TYPE")
								While RS.Read%>
									<tr>
					...
					<div class=LST>
						<table>
							...
							<%
							RS = getRecord("SELECT * FROM SYS_LEVEL(nolock) ORDER BY LVL_ID")
							While RS.Read  %>
								<tr>
					...
					<div class="LST GAP">
						<table>
							...
							<%
							RS = getRecord("SELECT * FROM SYS_BIAS(nolock) ORDER BY CASE COUNTRY WHEN 'XX' THEN 1 ELSE 0 END, COUNTRY, TYPE")
							While RS.Read			%>
								<tr>
								<% If RS(0) <> "XX" Then %>
					...
					<div class="LST GAP" style="width:350px">
						<table>
							...
							<%
							RS = getRecord("SELECT * FROM SYS_FVT(nolock) ORDER BY CASE COUNTRY WHEN 'XX' THEN 1 ELSE 0 END, COUNTRY, TYPE")
							While RS.Read		  %>
								<tr>
								<% If RS(0) <> "XX" Then %>
									<td><img align=absmiddle class=FLG src="/img/<%= RS(0) %>.jpg"> <img align=absmiddle src="/img/<%= RS(1) %>.png">
					...
					<div class="LST GAP" style="width:270px">
						...
						<%	RS = getRecord("SELECT * FROM SYS_CODE(nolock) WHERE TYPE LIKE 'TES_[VNQ]%' ORDER BY CHARINDEX(LEFT(NAME,1),'VNQ')")
						While RS.Read		  %>
						  <tr>
							<td><%= RS(2) %>
							<td><select name=<%= RS(0) %> style="width:92%"><%= makeList("SELECT", "", "SYS_CODE", "TES_LST", "CODE", "NAME", RS(1)) %></select> <%
						End While
						RS.Close %>
						</table>
					</div>
		...
		<!--  WISE	-->
		<% IF Session("LVL") = 5 OR Session("LVL") < 2 Then %>
			<form  method=post>
				<div id=CNT style="top:25px; left:1125px">
							...
							<div class=LST>
								<table>
									...
									<%
									RS = getRecord("SELECT * FROM SYS_WISE(nolock) ORDER BY CUST_ID, REGION")
									While RS.Read  %>
										<tr>
											<td><b><%= RS(1) %>
											<td><b><%= RS(2) %>
