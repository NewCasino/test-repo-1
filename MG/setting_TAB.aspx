				...
				<table width=1100 cellpadding=0>
					...
					<div class="LST GAP" id=WIP>
					    <table>
					        ...
					        <%
							RS = getRecord("SELECT * FROM SYS_ENUM_TAB (nolock) WHERE TYPE LIKE 'MKTPCT_%' ORDER BY [TYPE]")
							While RS.Read  %>
					        <tr>
					...
					<div class="LST GAP" id=WIP>
						<table>
							...
							<tr>
								<td colspan=3 class=SPT><%
								RS = getRecord("SELECT * FROM SYS_ENUM_TAB (nolock) WHERE TYPE IN( 'WISECL_A',  'WISECL_B', 'WISECL_C',  'WISECL_D')")
 								While RS.Read  %>
									<tr>
										<td colspan=2><%= RS(2) %>
										<td><input name=<%= RS(0) %> type=number  step='0.01' value=<%= FormatNumber(RS(1) / 100)  %>><% 
