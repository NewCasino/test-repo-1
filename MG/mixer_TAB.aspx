...
Sub Page_Load()
  ...
  Case "Save"
    S = ""
	RS = getRecord("SELECT * FROM SYS_MIXER_TAB")
    While RS.Read()
		N = ""
		For I = 6 To 32 
			DBcol = RS.GetName(I)
		...
		Next:
		S &= "UPDATE SYS_MIXER_TAB SET" & Mid(N, 2) & " WHERE ROW_ID=" & RS(0) & vbLf
    End While 
	RS.Close() 
...
<html>
	...
	<script>
		top.setTitle("Price Blend TAB Setting"); 
	...
	<body>
		<div id=CNT class=WD>
			<form method=post>
				<%
				RS = getRecord("SELECT * FROM SYS_MIXER_TAB ORDER BY TYPE DESC, (CASE REGION WHEN 'M' THEN 0 WHEN 'P' THEN 1 ELSE 2 END), VENUE, RANK, CONF_LVL")
					...
					<div class="LST GAP">
						...
						<%  While MOF AndAlso T = RS("TYPE")
								If MOF Then
									If G <> RS("REGION") Then %>
										<tr class=SML>
											<th colspan=4>
											<th>BOB<th>WOW
											<th>APN<th>NZ<th>LXB<th>QLD<th>365<th>LAD<th>IAS<th>SPB<th>PAL<th>UNI<th>TOP<th>EZY
											<th>VWM<th>xB1
							...
							<% 		End If
								...
								Next %>
								<td class=FF><%= BlendTotal %><td class=SIP><input type=number name="TAB_PCT_<%= RS("ROW_ID") %>" value=<%= sN0(RS("TAB_PCT")) %>><%
								MOF = RS.Read
							End While %>
						  </table>
					  </div>
