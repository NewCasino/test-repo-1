  ...
  If CM <> "" Then
    ...
    Case "Save"
      Dim RM As Object = getRecord("SELECT MEETING_ID FROM MEETING(nolock) " & _
        "WHERE MEETING_DATE='" & DT & "' AND COUNTRY IN('AU','NZ') AND TYPE =('R')")
...
Else
  ...
  Dim RM As Object = getRecord("SELECT MEETING_ID, TYPE, COUNTRY, VENUE FROM MEETING(nolock) " & _
    "WHERE MEETING_DATE='" & DT & "' AND COUNTRY IN('AU','NZ') AND TYPE =('R') ORDER BY TYPE DESC, COUNTRY, VENUE")
  While RM.Read()
      ...
      If RV("STATUS") = "DONE" Then
        ...
        For I As Byte = 0 To 3: If sNN(RV(I)) Then
          For Each R As String In RV(I).ToString.Split(vbLf): If R <> "" And Left(R, 1) <> "#" Then
            ...
            DR = DV.Rows.Find({Left(C(2), 1), C(0)}): If Not IsNothing(DR) Then TD(0, 2) += Val(C(4)) * DR(Right(RV.GetName(I), 3))
          End If: Next
        End If: Next
        ...
        For I As Byte = 4 To 6: If sNN(RV(I)) Then
          For Each R As String In RV(I).ToString.Split(vbLf): If R <> "" And Left(R, 1) <> "#" Then
            ...
            DR = DV.Rows.Find({Left(C(1), 1), C(0)}): If Not IsNothing(DR) Then TD(1, 2) += Val(C(3)) * sN0(DR(Right(RV.GetName(I), 3)))
          End If: Next
        End If: Next
